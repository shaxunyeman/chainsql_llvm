...
Error WasmObjectFile::parseChainSQLABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   chainsql_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("chainsql abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".chainsql_abi") {
     if (Error Err = parseChainSQLABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".chainsql_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".chainsql_notify") {
